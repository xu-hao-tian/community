...
public class UserService implements CommunityConstant {
    ...
    public Map<String, Object> updatePassword(User user, String oldPassword, String newPassword, String confirmPassword) {
        ...
        if (oldPassword.equals(newPassword)) {
            map.put("newPasswordMsg", "旧密码与新密码不能一致!");
            return map;
        }
    ...
    public void sendCode(String email, HttpServletResponse response) {
        ...
        // 将验证码存入redis，失效时间为10min
        String redisKey = RedisKeyUtil.getCodeKey(codeOwner);
        redisTemplate.opsForValue().set(redisKey, code, 60*10, TimeUnit.SECONDS);

        // 激活邮件，使用Thymeleaf创建的对象携带变量
        Context context = new Context();
        context.setVariable("email", email);
        context.setVariable("text", code);
        // 使用模板引擎，利用thymeleaf,将context放到/mail/activation.html文件中,然后利用mail包发送给邮箱
     ...
     * @return
     */
    public Map<String,Object> updatePasswordByCode(String email, String password) {
        Map<String, Object> map = new HashMap<>();
        // 验证密码
        if (StringUtils.isBlank(password)) {
            map.put("passwordMsg", "密码不能为空!");
